...
|[`6-6-compile.gdbnb`](./examples/6-6-compile.gdbnb)|`compile code`を使って，プログラムを埋め込む|
...
|ファイル名|内容|
|--|--|
|[`7-1-whatis1.gdbnb`](./examples/7-1-whatis1.gdbnb)|`whatis`で型を素早く知る(1)|
|[`7-1-whatis2.gdbnb`](./examples/7-1-whatis2.gdbnb)|`whatis`で型を素早く知る(2)|
|[`7-2-ptype.gdbnb`](./examples/7-2-ptype.gdbnb)|`ptype`で型の詳細を知る|

### アセンブリ言語レベルのデバッグ

|ファイル名|内容|
|--|--|
|[`8-1-break-byaddr.gdbnb`](./examples/8-1-break-byaddr.gdbnb)|アドレス指定でブレークする|
|[`8-2-stepi-nexti.gdbnb`](./examples/8-2-stepi-nexti.gdbnb)|`stepi`と`nexti`で，機械語命令単位でステップ実行する|
|[`8-3-register.gdbnb`](./examples/8-3-register.gdbnb)|レジスタの値を読んだり書いたりする|
|[`8-4-x1-endian.gdbnb`](./examples/8-4-x1-endian.gdbnb)|`x`でリトルエンディアンでの格納を確認する|
|[`8-4-x2-pointer.gdbnb`](./examples/8-4-x2-pointer.gdbnb)|`x`でポインタと実体の違いを確認する|
|[`8-4-x3-array.gdbnb`](./examples/8-4-x3-array.gdbnb)|`x`で配列の中身を確認する|
|[`8-4-x4-struct.gdbnb`](./examples/8-4-x4-struct.gdbnb)|`x`で構造体の中身を確認する|
|[`8-4-x5-instruction.gdbnb`](./examples/8-4-x5-instruction.gdbnb)|`x`で機械語命令の中身を確認する|
|[`8-4-x6-frame.gdbnb`](./examples/8-4-x6-frame.gdbnb)|`x`でスタックフレームのレイアウトを確認する|
|[`8-4-x7-buffer-overflow.gdbnb`](./examples/8-4-x7-buffer-overflow.gdbnb)|`x`でバッファオーバーフローを確認する|
|[`8-5-find.gdbnb`](./examples/8-5-find.gdbnb)|`find`で特定の値をメモリ中から探す|
|[`8-6-info-symbol.gdbnb`](./examples/8-6-info-symbol.gdbnb)|`info symbol`，`info address`，`info line`でシンボルテーブル情報を確認する|

### ヘルプ，アプロポス，補完

|ファイル名|内容|
|--|--|
|[`9-1-help.gdbnb`](./examples/9-1-help.gdbnb)|`help`でコマンドの説明を表示する|
|[`9-2-apropos.gdbnb`](./examples/9-2-apropos.gdbnb)|`apropos`で正規表現にマッチするコマンドを表示する|
|[`9-3-completion.gdbnb`](./examples/9-3-completion.gdbnb)|補完とヒストリ機能を使ってみる|
|[`9-4-user-defined.gdbnb`](./examples/9-4-user-defined.gdbnb)|ユーザ定義コマンドを使ってみる|


### よくあるバグのデバッグ

- segmentation fault
- 未初期化変数 -Wall -Wextra, info locals
- セグフォしないバッファオーバーフロー
- セグフォせず，スタック破壊
  watch *(long*)$rbp
- ダングリングポインタ
