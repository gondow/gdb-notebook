...
<details>
<summary>セルの実行の図</summary>

![説明図](https://gondow.github.io/linux-x86-64-programming/figs/execute-cell.png)
</details>

- ターミナル上で，同じコマンドを手動で入力しても同じ実行もできます．
- ツールチップ，コードレンズ，コマンド補完などで，コマンドの説明を見れます．

<details>
<summary>コマンド補完の図</summary>

![](images/gdbnb-completion.png)
</details>

<details>
<summary>ツールチップの図</summary>

![](images/gdbnb-tooltip.png)
</details>

## 使い方

- GitHub Codespacesを使う方法が一番簡単です(AMD64 Linux環境を自分で準備する必要が無いので)．詳しくは以下を参照下さい．
  - - [`HOWTOUSE_CODESPACE.md`](HOWTOUSE_CODESPACE.md)
- 以下の内容を`~/.gdbinit`に書き込むことをお勧めします．Github Codespacesをお使いの方は，自動でこの設定をしているので不要です．
```
# 確認プロンプトを出さない
set confirm off
# 出力をページ分割しない
set pagination off
# debuginfoの自動ダウンロードを無効化
...
- GDBノートブックごとに個別のターミナルを起動します．ノートブックごとに実行状態を別々に保つためです．VSCodeの仕様上，GDBノートブックを閉じても，ターミナルが残ることがあります．不要なターミナルは手動で消去して下さい．
